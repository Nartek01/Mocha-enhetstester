/*
Dess nedanstående funktionerna är mocha's rättelse funktioner medans require('../blackjack') specifiera att den ska jämföra den här filen (blackjack.spec.js) me filen utanför den här mappen (blackjack.js),
sen så ska blackjack.js köras och jämför svaret med vad blackjack.spec.js.
*/
const assert = require('assert'), //Om ett utryck blir 0 eller false så kommer programmet att stänga ner sig själv. Ett utryck i javascript kan vara "var","alert","if() { }","result = true".
  { createDeck, draw } = require('../blackjack') //importera blackjack.js och funktionerna draw(),createDeck().
  describe('blackjack.js', function () { /*
  Describe i samband med it() utgör ett såkallade definering eller gruppering av vad det är mocha ska leta efter, i den ovanstående fallen så är det en fil och en function (blackjack,js och function ())
  */
